...
const renderDocuments = () => {
  ...
  const documentsAPI = (url = 'https://localhost:44365/api/files/') => {
    return {
      fetchAll: () => _node_modules_axios_index__WEBPACK_IMPORTED_MODULE_0___default.a.get(url),
      findById: id => _node_modules_axios_index__WEBPACK_IMPORTED_MODULE_0___default.a.get(url + id),
  ...
  const iconExtension = extension => {
    ...
    } else if (extension.includes('jpg') || extension.includes('png') || extension.includes('jpeg')) {
  ...
  const renderDocuments = () => {
    documentsAPI().fetchAll().then(res => {
      ...
      documents.items.forEach(item => {
        ...
        iconItem.innerHTML = item.extension ? iconExtension(item.extension) : `<i class="file-icon fa fa-folder"></i>`;
        const fileName = document.createElement('td');
        fileName.innerText = item.name;
        const modifiedAt = document.createElement('td');
        modifiedAt.innerText = new Date(item.modifiedAt).toLocaleString();
        ...
        detailsButton.addEventListener('click', () => showDetails(item.id));
  ...
  const addFile = () => {
    ...
    if (fileName === '') {
      ...
      documentsAPI().create(newFile).then(res => {
        console.log(res);
      }).catch(err => {
        console.log(err);
      });
  ...
  const uploadFile = () => {
    ...
    documentsAPI().uploadFile(formData).then(res => {
      console.log(res);
    }).catch(err => {
      console.log(err);
    });
  ...
  const deleteFile = id => {
    if (confirm('Do you want to delete this file ?') == true) {
      documentsAPI().delete(id).then(res => {
        console.log(res);
      }).catch(err => {
        console.log(err);
      });
    ...
    documentsAPI().findById(id).then(res => {
      ...
      const renderFile = `
            <section id="name">
              ...
              <input id="modifiedAt" value="${new Date(res.data.modifiedAt).toLocaleString()}" type="text" disabled />
              <label>Modified By:</label>
              <input id="modifiedBy" value="${new Date(res.data.modifiedBy).toLocaleString()}" type="text" disabled />
            </section>
      ...
      document.querySelector('#btn-delete').addEventListener('click', () => deleteFile(res.data.id));
      document.querySelector('#btn-update').addEventListener('click', () => updateFile(res.data.id, res.data));
    }).catch(err => {
      console.log(err);
    });
  };
